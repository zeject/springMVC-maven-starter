...
public class JedisUtil {
    ...
    public static void main(String[] args) {
        ...
        // shardInfo.setPassword(Constants.REDIS_PASS);
        ...
        // ju.flushAll();
        /*
		 * for (int i = 0; i < 20; i++) { ju.set(i + "test", i + "-----------");
		 * } long d = System.currentTimeMillis(); System.out.println(d - c); for
		 * (int i = 20; i < 40; i++) { ju.set(i + "test", i + "-----------"); }
		 * long u = System.currentTimeMillis(); System.out.println(u - d);
		 */
    }
    ...
    public void ListOperate() {
        ...
        jedis = pool.get(this.db).getResource();
    ...
    public static Map<Integer, JedisPool> pool = new HashMap<Integer, JedisPool>();
    ...
    public JedisUtil(int db) {
        this.db = db;
        if (this.pool.get(db) == null) {
            JedisPoolConfig config = new JedisPoolConfig();
            // 控制一个pool可分配多少个jedis实例，通过pool.get(this.db).getResource()来获取；
            ...
            this.pool.put(db,
                    new JedisPool(config, Constants.REDIS_IP, Constants.REDIS_PORT, 10000, Constants.REDIS_PASS, db));
        }
    }
    ...
    public String flushAll() {
        ...
        try {
            ...
            pool.get(this.db).returnBrokenResource(jedis);
            logger.error("", e);
            e.printStackTrace();
        } finally {
            returnResource(pool.get(this.db), jedis);
        }
        return value;
    }
    ...
    /**
     * 对特定的hmset特定的字段进行原子更新
     *
     * @param key   redis 存储的key
     * @param field 需要更新的字段名
     * @param value 原子更新值
     * @return Long 返回更新成功后的新值，失败的话返回 null
     * @method: hincrBy() -by fjt
     */

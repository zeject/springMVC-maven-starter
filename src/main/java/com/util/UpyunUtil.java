...
public class UpyunUtil {
	 ...
	 * @return 是否上传成功
	 */
	public static boolean writeFile(String filePath, String realPath) {
		boolean b = false;
		try {
			UpYun upyun = new UpYun();
			File file = new File(realPath);
			b = upyun.writeFile(filePath, file, true);
		} catch (IOException e) {
			logger.error("", e);
			e.printStackTrace();
		}
	 ...
	 *//** 本地待上传的测试文件 *//*
						 ...
						 * 
						 * 
						 * static { File picFile = new File(SAMPLE_PIC_FILE);
						 * 
						 * if (!picFile.isFile()) {
						 * System.out.println("本地待上传的测试文件不存在!"); } } private
						 * static UpYun upyun = null;
						 * 
						 * public static void main(String[] args) throws
						 * Exception {
						 * 
						 * // 初始化空间 upyun = new UpYun(BUCKET_NAME,
						 ...
						 * 
						 * // 设置连接超时时间，默认为30秒 // upyun.setTimeout(60);
						 * 
						 * // 设置是否开启debug模式，默认不开启 upyun.setDebug(false);
						 * writeFile("/pic/web/ccc.jpeg", SAMPLE_PIC_FILE); //
						 * ****** 可选设置 end ******
						 * 
						 * 
						 ...
						 * 
						 * 
						 * // // 1.上传文件（文件内容） // testWriteFile(); // // //
						 * 2.图片做缩略图；若使用了该功能，则会丢弃原图 // testGmkerl(); // // //
						 * 3.图片旋转；若使用了该功能，则会丢弃原图 // testRotate(); // // //
						 * 4.图片裁剪；若使用了该功能，则会丢弃原图 // testCrop();
						 * 
						 * }
						 */
	 ...
	 * 
	 * // 要传到upyun后的文件路径 String filePath = DIR_ROOT + PIC_NAME;
	 * 
	 * // 本地待上传的图片文件 File file = new File(SAMPLE_PIC_FILE);
	 * 
	 * // 设置待上传文件的 Content-MD5 值 // 如果又拍云服务端收到的文件MD5值与用户设置的不一致，将回报 406
	 * NotAcceptable 错误 upyun.setContentMD5(UpYun.md5(file));
	 * 
	 * // 设置待上传文件的"访问密钥" // 注意： //
	 * 仅支持图片空！，设置密钥后，无法根据原文件URL直接访问，需带URL后面加上（缩略图间隔标志符+密钥）进行访问 // 举例： //
	 * 如果缩略图间隔标志符为"!"，密钥为"bac"，上传文件路径为"/folder/test.jpg"， //
	 * 那么该图片的对外访问地址为：http://空间域名 /folder/test.jpg!bac
	 * upyun.setFileSecret("bac");
	 * 
	 * // 上传文件，并自动创建父级目录（最多10级） boolean result = upyun.writeFile(filePath, file,
	 * true);
	 * 
	 * System.out.println(filePath + " 上传" + isSuccess(result));
	 * 
	 * // 获取上传文件后的信息（仅图片空间有返回数据） System.out.println("\r\n****** " +
	 * file.getName() + " 的图片信息 *******"); // 图片宽度 System.out.println("图片宽度:" +
	 * upyun.getPicWidth()); // 图片高度 System.out.println("图片高度:" +
	 * upyun.getPicHeight()); // 图片帧数 System.out.println("图片帧数:" +
	 * upyun.getPicFrames()); // 图片类型 System.out.println("图片类型:" +
	 * upyun.getPicType());
	 * 
	 * System.out.println("****************************************\r\n");
	 * 
	 * System.out.println("若设置过访问密钥(bac)，且缩略图间隔标志符为'!'，则可以通过以下路径来访问图片：");
	 * System.out.println(URL + filePath + "!bac"); System.out.println(); }
	 * 
	 *//**
		 * 图片做缩略图
		 * <p>
	 ...
	 * 
	 * // 设置缩略图的质量，默认 95 params.put(PARAMS.KEY_X_GMKERL_QUALITY.getValue(),
	 * "95");
	 * 
	 * // 设置缩略图的锐化，默认锐化（true） params.put(PARAMS.KEY_X_GMKERL_UNSHARP.getValue(),
	 * "true");
	 * 
	 ...
	 * 
	 * // 要传到upyun后的文件路径 String filePath = DIR_ROOT + "rotate.jpg";
	 * 
	 * // 本地待上传的图片文件 File file = new File(SAMPLE_PIC_FILE);
	 * 
	 * // 图片旋转功能具体可参考：http://wiki.upyun.com/index.php?title=图片旋转 Map<String,
	 ...
	 *//**
		 ...
		 * 
		 * // 要传到upyun后的文件路径 String filePath = DIR_ROOT + "crop.jpg";
		 * 
		 * // 本地待上传的图片文件 File file = new File(SAMPLE_PIC_FILE);
		 * 
		 * // 图片裁剪功能具体可参考：http://wiki.upyun.com/index.php?title=图片裁剪 Map<String,
		 * String> params = new HashMap<String, String>();
		 * 
		 * // 设置图片裁剪，参数格式：x,y,width,height
		 * params.put(PARAMS.KEY_X_GMKERL_CROP.getValue(), "50,50,300,300");
		 * 
		 * // 上传文件，并自动创建父级目录（最多10级） boolean result = upyun.writeFile(filePath,
		 * file, true, params);
		 * 
		 ...
		 * System.out.println(); }
		 * 
		 * private static String isSuccess(boolean result) { return result ?
		 * " 成功" : " 失败"; }
		 */
}